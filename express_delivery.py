...
def checkio(field_map):
    ...
    for i in permutations(boxes, 2):
        unpack_routes[i] = [shortest_path(graph, start_point, i[0]),
                            'B' + shortest_path(graph, i[0], i[1]) + 'B',
                            shortest_path(graph, i[1], end_point)]
    unpack_routes_metric = [
        len(i[0]) * 2 + len(i[1]) + len(i[2]) * 2 for i in sorted(unpack_routes.values())]
...
if __name__ == '__main__':
    ...
    assert check_solution(
        checkio, 12, ["S...", "....", "B.WB", "..WE"]), "1st Example"
    assert check_solution(
        checkio, 11, ["S...", "....", "B..B", "..WE"]), "2nd example"
